...
const initialRocketAngle = 3; // deg
...
let E12EstesThrust = [];
for (let i = 0; i < E12EstesThrustPre.length; i += 1) {
  E12EstesThrust.push(E12EstesThrustPre[i]);
}

console.log("go");
for (let i = 0; i < F15EstesThrust.length; i += 1) {
  console.log(i);
  E12EstesThrust.push([
    F15EstesThrust[i][0] + E12EstesThrustPre[E12EstesThrustPre.length - 1][0],
    F15EstesThrust[i][1]
  ]);
}

console.log(E12EstesThrust);
...
const thrustCurveData = E12EstesThrust;

//initial motor Mass. E12 is 62 grams F15 is 95 grams
const motorMassStart = 0.116; //kg
const motorMassLossRatio = 0.06; // how much Mass motor looses
...
const dragCoefficient = 0.65;
const dragArea = 0.00456; // m ^ 2
...
const PIDLoopTime = 0.005; // sec

// Use these to add noise and offset to the angle measurement
// of the rocket.
const angleSensorNoiseAmplitude = 0.01; // deg
...
let constantTorqueBias = 0.0001; // N*m
...
const windSinAmplitude = 0.0003; // N*m
const windSinFrequency = 1; // hz
